package com.ricedotwho.mcprotocol.protocol.packet.status.clientbound;
...
@Setter
@AllArgsConstructor
public class ClientboundStatusResponsePacket extends Packet {
    ...
    @With
    private @NonNull JsonObject data;

    public ClientboundStatusResponsePacket(ByteBuf data) {
        super(data);
    }

    public ClientboundStatusResponsePacket(Packet packet) {
    ...
    public ServerStatusInfo parseInfo() {
        ...
        return new ServerStatusInfo(description, players, version, null, icon, enforcesSecureChat);
    }
    ...
    private static JsonObject toJson(ServerStatusInfo info) {
        ...
        if (info.getIconPng() != null) {
            obj.addProperty("favicon", info.getIconPng());
        } else if (info.getIconBytes() != null) {
            obj.addProperty("favicon", iconToString(info.getIconBytes()));
        }
