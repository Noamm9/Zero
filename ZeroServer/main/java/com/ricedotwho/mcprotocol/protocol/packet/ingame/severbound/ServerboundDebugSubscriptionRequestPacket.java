package com.ricedotwho.mcprotocol.protocol.packet.ingame.severbound;

import com.ricedotwho.mcprotocol.protocol.packet.Packet;
import io.netty.buffer.ByteBuf;
import lombok.AllArgsConstructor;
import lombok.Getter;
import lombok.Setter;
import org.geysermc.mcprotocollib.protocol.codec.MinecraftTypes;
import org.geysermc.mcprotocollib.protocol.data.game.debug.DebugSubscriptions;

import java.util.ArrayList;
import java.util.List;

@Getter
@Setter
@AllArgsConstructor
public class ServerboundDebugSubscriptionRequestPacket extends Packet {

    private List<DebugSubscriptions> subscriptions;

    public ServerboundDebugSubscriptionRequestPacket(ByteBuf data) {
        super(data);
    }

    public ServerboundDebugSubscriptionRequestPacket(Packet packet) {
        super(packet.getRawData());
    }

    @Override
    public void decode(ByteBuf in) {
        this.subscriptions = new ArrayList<>();
        int length = MinecraftTypes.readVarInt(in);
        for (int i = 0; i < length; i++) {
            this.subscriptions.add(DebugSubscriptions.from(MinecraftTypes.readVarInt(in)));
        }
    }

    @Override
    public void encode(ByteBuf out) {
        MinecraftTypes.writeVarInt(out, this.subscriptions.size());
        for (DebugSubscriptions subscription : this.subscriptions) {
            MinecraftTypes.writeVarInt(out, subscription.ordinal());
        }
    }
}
